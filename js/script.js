...
if (!get('cursor')) ls.brush = 'cursor';
if (!ls.brush || isFinite(ls.brush)) { ls.sf = ls.smooth = ls.cap = 0;
ls.brush = 'cursor';
ls.imgwidth = 1000;
ls.imgheight = 1000;
ls.imgx = -500;
...
elem.img = images.length-1;
      var c = elem.index;
      if (ls.brush != 'pencil' && ls.brush != 'line' && ls.brush != 'cursor' && elem.cons != KeepDraw.Line) {
        elem = elem.toLine();
...
if (!selact && !run) {
  if (!move) {
    if (!selpoint) {
      if (ls.brush != 'eraser') clearsel();
      removed = [];
      move = !0;
      if (brushes[ls.brush][0] && no1) brushes[ls.brush][0](e);
    }
  }
...
var clientY1 = (((e.touches) ? e.touches[1].clientY : e.clientY) - get('main').offsetTop) * window.devicePixelRatio;
  ...
  if (move && !selpoint && !selact) {
    if (elem && ls.brush != 'cursor' && ls.brush != 'fill' &&ls.brush != 'pencil') elem.smooth(ls.smooth);
    if (brushes[ls.brush][1]) brushes[ls.brush][1](e, clientX0, clientY0);
  } else if (selpoint) {
    var seg = elem._segments || elem.segments;
