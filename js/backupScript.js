...
http.open('GET', 'http://noopdie.com:5000/wb_1')
http.send()
http.onload = () => {
	try {
var wb = JSON.parse(http.responseText);
for (var i = 0; i < wb.line.length; i++) {
	wb.line[i].x += innerWidth / 2  * window.devicePixelRatio;
  wb.line[i].y += innerHeight / 2  * window.devicePixelRatio;
  wb.line[i].stage = stage;
figures[wb.line[i].id] = new KeepDraw.Line(wb.line[i]);
}
} catch(err) { alert(err) }
}
...
var socket = new WebSocket('ws://noopdie.com:7000');
...
function sendElem(elem, id) {
    ...
    for (var key in attrs) {
       ...
       coords.strokeWidth = elem.strokeWidth / gZoom
     coords.segments = KeepDraw.scaleSegments(KeepDraw.rotateSegments(elem.segments, -gDeg, togRotX, togRotY), 1/gZoom, togRotX, togRotY);
