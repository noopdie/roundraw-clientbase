 ...
 var KeepDraw = {
   ...
   Utils: {
     draw: function(arg) {
       ...
       for (var k = 0; k < arg.layers.length; k++) {
       	for (var i = 0; i < arg.layers[k].childs.length; i++) {
         if (arg.layers[k].childs[i]) arg.layers[k].childs[i].draw(arg.layers[k].childs[i]);
         else crush = !0;
       }
       if (crush) {
         var ch = [];
         for (var i = 0; i < arg.layers[k].childs.length; i++) {
           if (arg.layers[k].childs[i]) ch.push(arg.layers[k].childs[i]);
         }
         arg.layers[k].childs = ch;
         arg.layers[k].setIndex();
     ...
     lineDraw: function(arg, point) {
       try {
         var stage = arg.stage || arg.layer.stage;
         stage.ctx.save();
         ...
         if (!arg.x) arg.x = 0;
         if (!arg.y) arg.y = 0;
         stage.ctx.translate(arg._x || arg.x, arg._y || arg.y);
         stage.ctx.lineTo(seg[0][0] + 0.5, seg[0][1] + 0.5);
         
         for (var i = 1; i < seg.length; i++) {
   if (typeof seg[i - 1][5]== 'number' && typeof seg[i - 1][4]== 'number' && typeof seg[i - 1][3]== 'number' && typeof seg[i - 1][2]== 'number') {
             stage.ctx.bezierCurveTo((seg[i - 1][2] + 0.5) >> 0, (seg[i - 1][3] + 0.5) >> 0, (seg[i - 1][4] + 0.5) >> 0, seg[i - 1][5], (seg[i][0] + 0.5) >> 0, (seg[i][1] + 0.5) >> 0);
           } else if (seg[i - 1][3]) {
             stage.ctx.quadraticCurveTo((seg[i - 1][2] + 0.5) >> 0, (seg[i - 1][3] + 0.5) >> 0, seg[i][0], (seg[i][1] + 0.5) >> 0);
           } else {
             stage.ctx.lineTo((seg[i][0] + 0.5) >> 0, (seg[i][1] + 0.5) >> 0);
           }
         }
         if (arg.closed) {
           stage.ctx.closePath();
         ...
         if (point) var inter = stage.ctx.isPointInPath(point[0], point[1]);
     ...
     setStyle: function(arg) {
     	if (!arg.stage) arg.stage = arg.layer.stage;
...
ry = ry || 0;
   ...
   this.stage.layers.push(this);
     ...
     for (var key in events) {
       this.stage.canvas.addEventListener(key, function(e) {
         for (var i = 0; i < events[e.type].length; i++) {
           var obj = events[e.type][i];
           if (KeepDraw.getIntersection(obj, (((e.touches) ? e.touches[0].clientX : e.clientX) - obj.layer.stage.canvas.offsetLeft) * window.devicePixelRatio, (((e.touches) ? e.touches[0].clientY : e.clientY) - obj.layer.stage.canvas.offsetTop) * window.devicePixelRatio )) {
             obj.on(e.type)(e, obj);
           }
         }
       }, false);
     }
     this.cons = KeepDraw.Layer;
     this.add = function(arg) {
       arg.layer = this;
       arg.cons(arg);
     };
     KeepDraw.Utils.draw(this.stage);
   },
   Line: function(arg) {
     var events = this.events = {};
     ...
     if (arg.stage || arg.layer) {
       this.draw = KeepDraw.Utils.lineDraw;
       this.draw(arg);
     }
     this.cons = KeepDraw.Line;
     if (arg.layer) {
       this.index = arg.layer.childs.length;
       arg.layer.childs.push(this);
     }
    else if (arg.stage) {
       this.index = stage.childs.length;
       stage.childs.push(this);
     }
   ...
   Text: function(arg) {
     ...
     if (stage) {
       this.draw = function(arg) {
         KeepDraw.Utils.setStyle(arg);
         if (arg.strokeColor) stage.ctx.strokeText(arg.text, arg._x || arg.x, arg._y || arg.y);
         if (arg.fill) stage.ctx.fillText(arg.text, arg._x || arg.x, arg._y || arg.y);
         if (arg.color) stage.ctx.fillText(arg.text, arg._x || arg.x, arg._y || arg.y);
         if (!arg.fill)
           if (!arg.stroke) stage.ctx.fillText(arg.text, arg._x || arg.x, arg._y || arg.y);
       };
     }
   ...
   scaleSegments: function(seg, scale, x, y) {
   	if (!y) x = 0, y = 0;
   	rseg = [];
           for (var i = 0; i < seg.length; i++) {
           	rseg[i] = [];
