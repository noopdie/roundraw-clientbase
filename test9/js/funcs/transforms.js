function transform(wb, _view) {
    ...
    for (var i = 0; i < wb.length; i++) {
        ...
        for (var k = 0; k < seg.length; k++) {
            ...
            //degX
            var degX = view_.degX / 360 * Math.PI * 2;
            var degY1 = view_.degY / 360 * Math.PI * 2;

            var degY = degY1 * Math.cos(degX);
            var degY2 = degY1 * Math.sin(degX);
            var deg = view_.deg / 360 * Math.PI * 2;
            var xd = (seg_[k][0] + wb[i].x) * Math.cos(degY2) - (seg_[k][1] + wb[i].y) * Math.sin(degY2);
            var yd = (seg_[k][0] + wb[i].x) * Math.sin(degY2) + (seg_[k][1] + wb[i].y) * Math.cos(degY2);
            seg_[k][0] = xd - wb[i].x, seg_[k][1] = yd - wb[i].y;
            var zx = -(seg_[k][0] + wb[i].x) * Math.sin(degX)  + (seg_[k][6] + wb[i].z) * Math.cos(degX);
