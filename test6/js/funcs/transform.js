function transform(wb, _view) {
    ...
    for (var i = 0; i < wb.length; i++) {
        ...
        for (var k = 0; k < seg.length; k++) {
            ...
            seg_[k][seg_[k].length-1] = {x: seg[k][seg[k].length-1].x, y: seg[k][seg[k].length-1].y} ;
            ...
            var zy = -(seg_[k][1] + wb[i].y) * Math.sin(degY)  + (seg_[k][seg_[k].length-1].x + wb[i].z - view_.z) * Math.cos(degY);
            var zx = -(seg_[k][0] + wb[i].x) * Math.sin(degX)  +  (seg_[k][seg_[k].length-1].y + wb[i].z - view_.z) * Math.cos(degX);
            ...
            var screenSize =  2 * ((innerHeight > innerWidth) ? innerHeight : innerWidth)  / view_.zoom;
            var dis = ((view_.deX) ? -1 : 1) * screenSize;
           
            if (true || !view_.deX) {
            var x2 = (dis * x1) / (zx + dis);
            var y2 = (dis * y1) / (zy + dis);
            } else {
                dis = -dis;
                var x2 = (dis * x1) / (zx + dis);
            ...
            seg_[k][seg_[k].length-1].x = zx - wb[i].z - (view.deZ) ? view.deZ : 0;
            seg_[k][seg_[k].length-1].y = zy - wb[i].z - (view.deZ) ? view.deg.deZ : 0;
