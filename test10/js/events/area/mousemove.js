get('area').onmousemove = function (e) {
    ...
    if (readings.ctrlKey && readings.move3D) {
      ...
      //var xRotated = ((x - readings.startX) / xAsix)  * Math.cos(view.deg) - ((y - readings.startY) / yAsix) * Math.sin(-view.deg);
       ...
       view.degX = readings.startDegX + (e.clientX - readings.startX) / ((innerWidth + innerHeight) / 2) * 360;
       view.degY = readings.startDegY + (e.clientY - readings.startY) / ((innerWidth + innerHeight) / 2) * 360;
        if (view.degX < 0) view.degX += 360;
        if (view.degX > 360) view.degX -= 360;
        if (view.degY < -90) view.degY = -90;
        if (view.degY > 90) view.degY = 90;
