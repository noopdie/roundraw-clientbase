get('area').addEventListener('touchmove', function (e) {
    if (e.touches[2] && !readings.touchDraw) {
        ...
        var xAsix = innerWidth * devicePixelRatio
       var yAsix = innerHeight * devicePixelRatio;
      //var xRotated = ((x - readings.startX) / xAsix)  * Math.cos(view.deg) - ((y - readings.startY) / yAsix) * Math.sin(-view.deg);
       //  var yRotated = ((x - readings.startX) / xAsix) * Math.sin(view.deg) + ((y - readings.startY) / yAsix) * Math.cos(-view.deg);
        view.degX = ((readings.startDegX/360) + (x - readings.startX) / xAsix) * 360;
        if (view.degX < 180) view.degX += 360;
        if (view.degX > 180) view.degX -= 360;
        //view.degY = ((readings.startDegY/360) + (y - readings.startY) / yAsix) * 360;
        ...
        var moveArea = (dis3 > dis4) ? dis3 : dis4;
        //var moveArea = calcPolygonArea({segments: [[e.touches[0].clientX, e.touches[0].clientY], [e.touches[1].clientX, e.touches[1].clientY], [e.touches[2].clientX, e.touches[2].clientY]]});
        var area = readings.startZ + innerHeight * (moveArea / readings.startArea) - innerHeight;
        view.z = area;
