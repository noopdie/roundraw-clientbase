...
if (wbid) {
    ...
    socket = { send: () => { } }
    var meta = window.location.href.split('?');
    var localwb = localStorage[(meta[1]) ? 'wb_' + meta[1] : 'wb']
    view.wb = (localwb) ? JSON.parse(localwb) : [];
    view.wb_ = (localwb) ? JSON.parse(localwb) : [];
    draw(view.wb);
}
...
window.onbeforeunload = function () {
    if (!wbid) {
        var meta = window.location.href.split('?');
        if (!meta[1]) { localStorage.wb = JSON.stringify(view.wb);
            localStorage.image = c.toDataURL();
        }
        else { localStorage['wb_' + meta[1]] = JSON.stringify(view.wb);
        localStorage['image_' + meta[1]] = c.toDataURL();
    }
    }
}
